...
function createInstance({ name, version, loader = 'vanilla', loaderVersion = null, memoryMB = null }) {
  ...
  // v1.0: no more pack injection — instead, if THIS dir ever got the old
  // injected NX-UI pack, clean it out the moment the instance is touched.
  try { require('./nx-inject').purgeInjectedPack(instanceGameDir(inst.id)); } catch {}
  return inst;
}
