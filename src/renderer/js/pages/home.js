...
export async function buildHome(container) {
  ...
  const renderGameStatus = () => {
    ...
    if (g.phase === 'running') {
      ...
      statusArea.append(
        el('div', { class: `status-banner ${bad ? 'warn' : 'done'}` },
          el('span', { class: 'dot' }),
          el('div', { class: 'status-text' },
            el('b', { text: bad ? `Minecraft exited with code ${g.exitCode}` : 'Minecraft session ended' }),
            el('span', { text: bad ? 'Something may have gone wrong — check the Logs.' : 'Ready when you are — hit PLAY again.' }),
          ),
        ),
      );
