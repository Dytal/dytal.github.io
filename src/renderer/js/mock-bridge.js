// mock-bridge.js — DEVELOPER UI-PREVIEW HARNESS. In the packaged launcher this
// file exits before doing anything (window.neurax always comes from preload.js)
// and outside Electron it activates ONLY with ?devmock in the URL. The fake
// data/delays here can therefore NEVER reach end users — production always runs
// the real engine. (Kept as a classic script so it evaluates BEFORE the ES modules.)
(function () {
  'use strict';
  if (window.neurax) return;                                          // real engine present (production)
  if (typeof navigator !== 'undefined' && /Electron/i.test(navigator.userAgent)) return; // electron w/o bridge → fail real, not fake
  if (!/[?&]devmock/.test(location.search)) return;                   // plain browser without opt-in
  ...
  const settings = {
    version: 4, theme: 'emerald', memoryMB: 4096, memoryAuto: true, smartInstall: true,
    neuraxClient: true, clientThirdPartyMods: false, clientFpsMode: 'ultra', javaPathOverride: '',
    closeLauncherOnLaunch: false, keysAcknowledgedAt: null, nxLegacyServer: false,
  ...
  window.neurax = {
    invoke: (channel, payload = {}) => {
      switch (channel) {
        case 'app:info': return delay({ version: '1.0.0-preview', neuraxRoot: 'C:/Users/Demo/AppData/Roaming/.neurax', platform: 'win32', totalMemoryMB: 16384 });
        case 'diag:summary': return delay({
          platform: 'win32',
          gpus: [
            { name: 'AMD Radeon (TM) Graphics', driver: '23.19.260413', type: 'integrated' },
            { name: 'NVIDIA GeForce RTX 3060', driver: '566.14', type: 'dedicated' },
          ],
          hasIntegratedGPU: true, hasDedicatedGPU: true,
          totalMB: 16384, safeMemoryCapMB: 11468,
          lastCrash: null, crashesRecent: 0, safeModeArmed: false,
        });
        case 'diag:armSafeMode': return delay({ armed: true });
        case 'diag:restorePacks': return delay({ results: [{ target: 'global', restored: 0, removed: 0 }] });
        case 'app:showFile': return delay(true);
        ...
        case 'auth:msWindowCancel': return delay(true);
        case 'auth:msCancel': return delay(true);
        case 'auth:msStart': {
          // demo: simulate the v4.3 ADVANCED device-code flow (custom Azure id only;
          // the built-in Minecraft app id is refused with AADSTS700016)
          (async () => {
            await new Promise(r => setTimeout(r, 700));
            emitEvent('auth:deviceCode', { userCode: 'PREVIEW-CODE', verificationUri: 'https://www.microsoft.com/link' });
          })();
          return delay({ type: 'msa', name: 'Steve', uuid: 'demo-uuid', accessToken: 'demo', skin: null }, 3200);
        }
        ...
        case 'nx:adminList': return delay([{ uuid: '00000000-0000-4000-8000-000000000001', deviceName: 'Demo-PC', playerName: 'Steve', playerType: 'msa', playerUuid: 'abcdef01-2345-6789-abcd-ef0123456789', appVersion: '4.0.0', instances: [], gameRunning: false, lastSeen: now, online: true, locked: false, lockReason: '', lockedUntil: null, blocked: false }]);
        case 'nx:blockList': return delay([]);
        ...
        // v4 preview stubs (dev harness only)
        case 'nx:chatDM': case 'nx:chatDelete': case 'nx:chatRename': case 'nx:chatSetRole':
        case 'nx:chatKick': case 'nx:chatStar': case 'nx:msgEdit': case 'nx:msgDeleteForMe':
        case 'nx:msgDeleteForEveryone': case 'nx:friendAdd': case 'nx:friendRemove': case 'nx:friendStar':
        case 'nx:voiceJoin': case 'nx:voiceTick': case 'nx:voiceSignal': case 'nx:voiceUpdate':
        case 'nx:voiceLeave': case 'nx:blockAdd': case 'nx:blockRemove': case 'nx:eventsList': return delay({});
        // v4.1 preview stubs (dev harness only) — identity editor + browser login stages
        case 'nx:identityRead': return delay({ uuid: '00000000-0000-4000-8000-000000000000', fingerprint: 'a'.repeat(64), source: 'preview', file: null, msClientId: '', cloudUuid: '00000000-0000-4000-8000-000000000000', appVersion: '4.1.0' });
        case 'nx:identityWrite': return delay({ applied: { uuid: payload.uuid, fingerprint: payload.fingerprint, previousUuid: '00000000-0000-4000-8000-000000000000', file: '(preview)' }, cloud: { uuid: payload.uuid, rebind: { ok: true, uuid: payload.uuid, moved: [] } }, identity: { uuid: payload.uuid, fingerprint: payload.fingerprint, source: 'owner-edit' }, msClientId: payload.msClientId || '' });
        case 'nx:mojangLookup': return delay({ uuid: '069a79f4-44e9-4726-a5be-fca90e38aaf5', name: payload.name || 'Notch' });
        case 'auth:msBrowser': {
          (async () => {
            emitEvent('auth:msBrowser', { stage: 'browser', message: 'PREVIEW: your default browser would open the Microsoft sign-in now.' });
            await new Promise(r => setTimeout(r, 900));
            emitEvent('auth:msBrowser', { stage: 'browser', message: 'PREVIEW: waiting for you to finish in the browser…' });
          })();
          return new Promise((resolve, reject) => setTimeout(() => reject(new Error('[preview] browser sign-in never completes in preview mode — use Cancel')), 6000));
        }
        case 'auth:msBrowserCancel': case 'auth:msWindowCancel': return delay(true);
        case 'nx:friendsList': return delay({ friends: [{ uuid: 'u2', name: 'Steve', type: 'msa', starred: true, online: true, lastSeen: now }] });
        case 'nx:blockList': return delay([]);
        ...
        // ---- v4.5 Owner Console (preview stubs — dev harness only) ----
        // v4.5: the key display channels (keysFirstRun/keysAcknowledge/
        // keysReveal/keysRotate) were REMOVED — the fixed owner key is never
        // shown in the UI. keysStatus stays metadata-only.
        case 'owner:keysStatus': return delay({ source: 'canonical', sourceLabel: 'Owner fixed key — sealed key vault (.neurax/keys.vault)', keysFile: 'C:/Users/Demo/AppData/Roaming/.neurax/keys.vault', ackedAt: settings.keysAcknowledgedAt, firstRunPending: false });
        case 'owner:listFiles': return delay({ root: 'C:/Users/Demo/AppData/Roaming/.neurax', files: [
          { name: 'keys.vault', dir: '', bytes: 214, mtime: now, sealed: true },
          { name: 'settings.vault', dir: '', bytes: 386, mtime: now, sealed: true },
          { name: 'vault.bin', dir: '', bytes: 287, mtime: now, sealed: true },
          { name: 'cc-vault.bin', dir: '', bytes: 214, mtime: now, sealed: true },
          { name: 'identity-cache.vault', dir: '', bytes: 195, mtime: now, sealed: true },
          { name: 'device-grant.vault', dir: '', bytes: 173, mtime: now, sealed: true },
          { name: 'instances.json', dir: '', bytes: 1420, mtime: now, sealed: false },
          { name: 'servers.json', dir: '', bytes: 980, mtime: now, sealed: false },
          { name: '.vk', dir: '', bytes: 32, mtime: now, sealed: false },
          { name: 'tokens.vault', dir: 'auth', bytes: 512, mtime: now, sealed: true },
          { name: 'device-identity.json', dir: 'identity', bytes: 160, mtime: now, sealed: false },
        ] });
        case 'owner:readFile': return delay({ name: payload.file?.split(/[\\/]/).pop() || 'file', sealed: true, kind: 'json', content: JSON.stringify({ _comment: 'PREVIEW — decrypted content of a sealed file', demo: true }, null, 2) });
        case 'owner:writeFile': return delay({ ok: true, sealed: true });
        case 'owner:unlockData': return delay({ releasedIdentity: ['C:/ProgramData/Neurax/device-identity.json'], unlocked: 14, root: 'C:/Users/Demo/AppData/Roaming/.neurax' });
        case 'owner:wipeAll': return delay({ identityAnchors: [], unlockedFiles: 14, trashed: [{ dir: 'C:/Users/Demo/AppData/Roaming/.neurax', ok: true }], errors: [] });
        // ---- v1.0 stubs (preview harness) ----
        case 'owner:verifyKey': return delay({ ok: true, persisted: true, grantedAt: now, keyHash: 'demo'.repeat(16) });
        case 'owner:grantStatus': return delay({ granted: false, autoGranted: false, source: null, persisted: false, persistedAt: null, keyMatches: null, grantFile: 'C:/Users/Demo/AppData/Roaming/.neurax/device-grant.vault' });
        case 'owner:revoke': return delay(true);
        case 'client:status': return delay({
          supportedVersions: ['26.3', '26.2', '26.1.2', '26.1.1', '26.1', '1.21.11'],
          instanceScope: ['26.1.2'], injectorVersion: 3,
          nx: { version: '2.0.0', file: 'nx-2.0.0.jar' },
          stack: [
            { slug: 'nx', name: 'NX', purpose: 'Neurax core: adaptive FPS engine', modes: ['balanced', 'ultra'] },
            { slug: 'sodium', name: 'Sodium', purpose: 'Rewrites the renderer', modes: ['balanced', 'ultra'] },
            { slug: 'lithium', name: 'Lithium', purpose: 'Tick logic optimizations', modes: ['balanced', 'ultra'] },
            { slug: 'c2me-fabric', name: 'C2ME', purpose: 'Parallel world gen', modes: ['ultra'] },
          ],
          cached: [], instances: [],
        });
        case 'client:inject': return delay({ ok: true, already: false, mods: 12 });
        case 'client:uninject': return delay({ ok: true, removedMods: 12 });
        case 'client:clearCache': return delay(true);
        case 'nx:invitesList': return delay({ pending: { friendRequests: [], chatInvites: [] }, connected: false });
        case 'nx:friendRespond': return delay({ ok: true, accepted: !!payload.accept });
        case 'nx:inviteRespond': return delay({ ok: true, accepted: !!payload.accept });
        case 'nx:packInfo': return delay({ pack: 'NX-UI-64x', exists: false, removed: true });
        default:
