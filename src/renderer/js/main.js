...
async function startInner() {
  ...
  api.on('launch:state', (p) => {
    ...
    } else if (p.state === 'stabilizing') {
      // v1.0 close-on-launch: the launcher waits until the game has proven it
      // is really up (12s alive) — only then does it quit. A game that dies
      // during startup can never close the launcher and vanish.
      if (g.phase !== 'running') setGameState({ phase: 'running', label: 'Minecraft starting — the launcher will close automatically once the game is up', pid: g.pid ?? null });
    } else if (p.state === 'handoff') {
      // v4.4 close-on-launch: the launcher quits itself — Minecraft keeps everything
      setGameState({ phase: 'running', label: 'Launcher closing — Minecraft has all resources now', pid: g.pid ?? null });
      toast('Launcher closing', 'Minecraft keeps running — Neurax reopens automatically when the game exits.', { timeout: 6000 });
    } else if (p.state === 'exited') {
      const stopped = g.stopped;
      setGameState({ phase: 'exited', exitCode: p.code ?? null, crash: p.crash || null, pid: null, lastProgress: null, stopped: false });
      if (stopped) toast('Minecraft was stopped', 'Session ended.');
      else if (p.crash) toast(p.crash.title || 'Minecraft crashed', p.crash.summary?.description || p.crash.detail || 'The GPU driver or the game itself crashed — a safe relaunch is one click away on the dashboard.', { type: 'error', timeout: 10000 });
  ...
  /* v4.6 notices from the crash doctor (safe mode armed / packs restored). */
  api.on('launch:notice', (p) => {
    if (p?.message) toast('Neurax', p.message, { timeout: 9000 });
  });
