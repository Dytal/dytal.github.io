#!/usr/bin/env python3
# build_v430_full.py — packages the COMPLETE Neurax Launcher project (v4.3.0)
...
# v4.3.0 additions to the verification battery:
#   - probe-v43.js must be present
#   - settings.js / auth.js / device-identity.js must reference the SEALED
#     files (settings.vault / tokens.vault / identity-cache.vault)
#   - auth.js must contain the AADSTS700016 device-code refusal
# Run:  python3 scripts/build_v430_full.py
...
NAME = 'Neurax-Launcher-v4.3.0-FULL-SOURCE.zip'
...
EXCLUDE_FILES = {
    ...
    'settings.vault', 'tokens.vault',    # local sealed runtime data (be safe)
}
EXCLUDE_EXT = {'.log', '.tmp'}
...
with zipfile.ZipFile(DEST) as z:
    ...
    for n in names:
        ...
        assert not n.endswith(('.vk', 'vault.bin', 'cc-vault.bin', 'settings.vault', 'tokens.vault')), f'runtime secret in zip: {n}'
    # 2. no baked secrets anywhere in the archive (skip the build-guard
    #    scripts themselves — build_v*_full.py files legitimately carry the
    #    BURNED old-key fragments as negative assertions; every other member
    #    is scanned in full)
    import re
    guard_re = re.compile(r'^neurax-launcher/scripts/build_v\d+_full\.py$')
    ...
    # 4. v4.3.0 feature markers inside the shipped sources
    settings_src = z.read('neurax-launcher/src/main/core/settings.js').decode('utf8')
    auth_src = z.read('neurax-launcher/src/main/core/auth.js').decode('utf8')
    ...
    assert 'settings.vault' in settings_src, 'settings.js must persist to the sealed file'
    assert 'tokens.vault' in auth_src, 'auth.js must persist tokens to the sealed file'
    assert 'identity-cache.vault' in ident_src, 'device-identity.js must seal the cache'
    assert 'AADSTS700016' in auth_src, 'device-code refusal for the built-in id missing'
    for fn_name in ('readSealed', 'writeSealed', 'shredPlaintext'):
        assert f'function {fn_name}' in vault_src and f'{fn_name},' in vault_src, f'vault primitive missing: {fn_name}'
