...
(async () => {
  ...
  ok(fs.existsSync(vaultFile), 'console vault file created in the .neurax root (cc-vault.bin)');
  {
    ...
    ok(!raw.includes(KEY) && !raw.includes('sINm6w7h8PfCelSnlirOhgTPq2wSd03X') && !raw.includes('Auiwadhagwid156'),
  ...
  r = await api('/api/login', { body: { pass: 'sINm6w7h8PfCelSnlirOhgTPq2wSd03X' } });
  ...
  ok(vmem.adminKey === 'sINm6w7h8PfCelSnlirOhgTPq2wSd03X' && typeof vmem.unlockPasskey === 'string' && vmem.unlockPasskey.length > 8,
  ...
  ok(ccSrc.includes("nx-admin-keys"), 'CC reads the baked owner admin key from nx-admin-keys.js');
  const keysSrc = fs.readFileSync(path.join(ROOT, 'src', 'main', 'core', 'nx-admin-keys.js'), 'utf8');
  ok(/ADMIN_KEY:\s*'[^']+'/.test(keysSrc) && /UNLOCK_PASSKEY:\s*'[^']+'/.test(keysSrc), 'nx-admin-keys.js exports the fixed ADMIN_KEY + UNLOCK_PASSKEY');
