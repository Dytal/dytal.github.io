...
// ---------------------------------------------------------------- scenario 1+2+3+10
{
  const r = run(`${H}
    ...
    console.log(JSON.stringify({ st, raw: require('fs').readFileSync(st.file).toString('latin1') }));
  })();`, '');
  ...
  ok(!j.raw.includes('sINm6w7h8PfCelSnlirOhgTPq2wSd03X'), 'admin key NOT stored in plaintext');
  ok(!j.raw.includes('Auiwadhagwid156'), 'unlock passkey NOT stored in plaintext');
